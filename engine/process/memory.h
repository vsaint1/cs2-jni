...
public:
  ...
  uintptr_t get_modulebase(std::string module_name) { return get_module(module_name); }

  int get_procid() { return process_id; }
  ...
  std::string read_str(uintptr_t address, size_t size) {
    std::string buffer(size, '\0');
    read_virtual(handle, (void *)address, &buffer, size, 0);
    return buffer;
  }
...
inline Memory process("cs2.exe"); // TODO: change to your desired process game name