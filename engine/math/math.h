...
static ULONG ww = GetSystemMetrics(SM_CXSCREEN); // width
static ULONG wh = GetSystemMetrics(SM_CYSCREEN); // height
...
class FVector2 {
	...
	FVector2(FVector2D vec2) : x(vec2.x), y(vec2.y) {}

	FVector2D operator=(FVector2D vec2) {
		x = vec2.x;
		y = vec2.y;
		return *this;
	}
	...
	float Distance(const FVector2& Pos) { return sqrtf(powf(Pos.x - x, 2) + powf(Pos.y - y, 2)); }
};
...
struct FVector3 {
	...
	FVector3 world_to_screen(ViewMatrix_t matrix) const {
    ...
    bool world_to_screen(const FVector3& pos, FVector3& out, ViewMatrix_t matrix) {
